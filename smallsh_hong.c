...
/*
char *getLineFILEorSTDIN(FILE* fp, size_t size){
	char *str;
	int ch;
	size_t len = 0;
...
int main(){
    ...
    SIGINT_action.sa_handler = Ctrl_C_handler;   // handler when caught
    ...
    SIGTSTP_action.sa_handler = Ctrl_Z_handler;   // handler when caught
    ...
    SIGCHLD_action.sa_handler = CatchSignal;   // handler when caught
  ...
  /* start shell activation */
    while (keeploop){
        ...
        //line = getLineFILEorSTDIN(stdin,MAXLINE);
        line = malloc(MAXLINE * sizeof(*line));
        fflush(stdin);
        fflush(stdout);
