...
void encrypt_Otp(char* Connection_Tx, const char* message, const char* key);
...
int main(int argc, char const *argv[]) {
  ...
  number_child = 0; // init number of child process
    ...
    if(get_ConnectionFD){
      ...
      if(getPid < 0){
        ...
        while(total_chars_read < message_len){
          ...
          if(get_recv < 0){
            fprintf(stderr, "ERROR: Empty reading !! :: port = %s\n", port_num );
            break;
          }
        ...
        encrypt_Otp(Connection_Tx, message, key);
        ...
        if( get_recv < 0 ){
          fprintf(stderr, "ERROR: sending ERROR:: get_recv %s\n", port_num );
          return 1;
        }
...
void encrypt_Otp(char* Connection_Tx, const char* message, const char* key){
  ...
  for(i = 0; i < message_len; i++){
    ...
    int Connection_index = strchr(GET_CHAR, message[i]) - GET_CHAR;
    Connection_index += strchr(GET_CHAR, key[i]) - GET_CHAR;
    Connection_index %= 27; // this is char range that GET_CHAR's range
    Connection_Tx[i] = GET_CHAR[Connection_index];
