...
* File Name : otp_enc.c
*	encrypt system input from file that generated
*/
#include <stdio.h>
#include <sys/types.h>
...
int main(int argc, char *argv[])
{
	...
	while(charsSignal <= msglength){
		bytesread = read(file_open, buffer, sizeof(buffer)-1);			//read data from the file.
	...
	file_open = open(argv[2], 'r');
	charsSignal = 0;
	while(charsSignal <= msglength){
		bytesread = read(file_open, buffer, sizeof(buffer)-1);
	...
	*/
	...
	while(charsSignal < msglength){
		charsSignal += recv(socketFD, buffer, sizeof(buffer)-1, 0); // getting message
		strcat(ciphertext, buffer); // recieved data will added in message
	}
	strcat(ciphertext, "\n");			// adding newline on the ciphertext
	printf("%s", ciphertext); 		// print ciphertext
