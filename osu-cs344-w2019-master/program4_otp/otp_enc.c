...
int main(int argc, char** argv) {
    ...
    unsigned char otp_type = ENCRYPTOR;
    ...
    WriteToServer(socket_fd, &plaintext_len, sizeof(plaintext_len), 0);
    // then send the plaintext to server
    WriteToServer(socket_fd, plaintext, plaintext_len, 0);
    ...
    ReadFromServer(socket_fd, &ciphertext_len, sizeof(ciphertext_len), 0);
    // then read ciphertext from server
    char ciphertext[ciphertext_len + 1];  // +1 for \0
    memset(ciphertext, '\0', sizeof(ciphertext));
    ReadFromServer(socket_fd, ciphertext, ciphertext_len, 0);
